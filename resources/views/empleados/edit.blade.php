...
@section('content')
    ...
    <form action="{{ route('empleados.update', $empleado->ID_empleado) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" id="ID_rol" name="ID_rol" value="{{ old('ID_rol', $empleado->ID_rol) }}" required>
        </div>
        <div class="mb-3">
            <label for="ID_sucursal" class="form-label">ID Sucursal</label>
            <input type="number" class="form-control" id="ID_sucursal" name="ID_sucursal" value="{{ old('ID_sucursal', $empleado->ID_sucursal) }}" required>
        </div>
        <div class="mb-3">
            <label for="ID_seccion" class="form-label">ID Sección</label>
            <input type="number" class="form-control" id="ID_seccion" name="ID_seccion" value="{{ old('ID_seccion', $empleado->ID_seccion) }}" required>
        </div>
