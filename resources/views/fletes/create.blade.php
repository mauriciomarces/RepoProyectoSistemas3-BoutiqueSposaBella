...
<div class="container">
    <div class="row mb-4">
        <div class="col">
            <h1><i class="fas fa-truck me-2"></i>Registrar Nuevo Flete</h1>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('fletes.store') }}" method="POST" id="fleteForm">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <div id="selectedClienteInfo" class="mt-3" style="display: none;">
                        <div class="alert alert-success">
                            ...
                            <span id="selectedClienteNombre"></span>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <textarea name="direccion"
                            ...
                            rows="2"
                            required
                            placeholder="Dirección completa de entrega">{{ old('direccion') }}</textarea>
                        @error('direccion')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="descripcion"
                        ...
                        rows="3"
                        maxlength="1000"
                        placeholder="Detalles adicionales sobre el flete (opcional)">{{ old('descripcion') }}</textarea>
                    @error('descripcion')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-primary" id="submitBtn" disabled>
                        <i class="fas fa-save me-2"></i>Guardar Flete
                    </button>
...
<script>
    ...
    function displayResults(clientes) {
        ...
        clientes.forEach(cliente => {
            html += `
            <a href="#" class="list-group-item list-group-item-action" onclick="selectCliente(${cliente.ID_cliente}, '${escapeHtml(cliente.nombre)}', '${escapeHtml(cliente.correo || '')}', '${escapeHtml(cliente.telefono || '')}', '${escapeHtml(cliente.direccion || '')}'); return false;">
    ...
    function selectCliente(id, nombre, correo, telefono, direccion) {
        ...
        if (!telefonoInput.value && telefono) {
            telefonoInput.value = telefono;
        }
        if (!direccionInput.value && direccion) {
    ...
    document.getElementById('fleteForm').addEventListener('submit', function(e) {
        if (!clienteIdInput.value) {
            e.preventDefault();
            alert('Por favor, seleccione un cliente antes de guardar el flete.');
            searchInput.focus();
        }
    });
</script>
